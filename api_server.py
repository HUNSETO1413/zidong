...
class WorkflowTranslator:
    ...
    def load_translations(self):
        ...
        try:
            with open('context/translations.json', 'r', encoding='utf-8') as f:
                self.translations = json.load(f)
        except FileNotFoundError:
            print("Warning: translations.json not found, using default English")
            self.translations = {}
    
    def translate_workflow_name(self, name: str, lang: str = 'en') -> str:
        """Translate workflow name to target language."""
        if lang == 'en' or lang not in self.translations:
            return name
        
        # For now, keep the original name as it's often a proper noun
        # Could be enhanced to translate common workflow patterns
        return name
    
    def translate_description(self, description: str, lang: str = 'en') -> str:
        """Translate workflow description to target language."""
        if lang == 'en' or lang not in self.translations:
            return description
        ...
        # Translate trigger descriptions
        for en_trigger, zh_trigger in trans.get('trigger_descriptions', {}).items():
            translated = translated.replace(en_trigger, zh_trigger)
        
        # Translate additional patterns
        for en_pattern, zh_pattern in trans.get('additional_patterns', {}).items():
            translated = translated.replace(en_pattern, zh_pattern)
        
        # Translate integration actions
        for en_action, zh_action in trans.get('integration_actions', {}).items():
            translated = translated.replace(en_action, zh_action)
        
        # Translate workflow purposes
        for en_purpose, zh_purpose in trans.get('workflow_purposes', {}).items():
            translated = translated.replace(en_purpose, zh_purpose)
        
        # Translate service names
        for en_service, zh_service in trans.get('common_services', {}).items():
            # Use word boundaries to avoid partial matches
            pattern = r'\b' + re.escape(en_service) + r'\b'
            translated = re.sub(pattern, zh_service, translated, flags=re.IGNORECASE)
        
        # Translate node count
        if 'nodes' in translated:
            translated = re.sub(r'(\d+)\s+nodes', r'\1 个节点', translated)
        
        # Translate "Uses X nodes"
        translated = re.sub(r'Uses\s+(\d+)\s+nodes', r'使用 \1 个节点', translated)
        
        # Translate "X services"
        translated = re.sub(r'(\d+)\s+services', r'\1 个服务', translated)
        
        return translated
    
    def translate_integration_name(self, integration: str, lang: str = 'en') -> str:
        """Translate integration/service name to target language."""
        if lang == 'en' or lang not in self.translations:
            return integration
        
        trans = self.translations[lang]
        return trans.get('common_services', {}).get(integration, integration)
    
    def translate_category_name(self, category: str, lang: str = 'en') -> str:
        """Translate category name to target language."""
        if lang == 'en' or lang not in self.translations:
            return category
        
        trans = self.translations[lang]
        return trans.get('categories', {}).get(category, category)
...
async def root(request: Request):
    """Serve the main documentation page with language selection support."""
    static_dir = Path("static")

    # Select lang by: query > cookie > IP country headers > Accept-Language
    lang = request.query_params.get("lang")
    if not lang:
        lang = request.cookies.get("lang")
    if not lang:
        country_code = (request.headers.get("cf-ipcountry") or request.headers.get("x-country-code") or "").upper()
        if country_code in {"CN", "HK", "MO", "TW", "SG"}:
            lang = "zh"
    if not lang:
        accept_lang = request.headers.get("accept-language", "").lower()
        lang = "zh" if accept_lang.startswith("zh") else "en"

    preferred = static_dir / ("index-zh.html" if lang == "zh" else "index.html")
    fallback = static_dir / "index.html"
    serve = preferred if preferred.exists() else fallback
...
async def search_workflows(
    ...
    lang: str = Query("en", description="Language for translation (en/zh)")
):
    ...
    try:
        ...
        for workflow in workflows:
            try:
                ...
                if lang == 'zh':
                    name = translator.translate_workflow_name(name, lang)
                    description = translator.translate_description(description, lang)
                    integrations = [
                        translator.translate_integration_name(integration, lang) 
...
async def get_categories(lang: str = Query("en", description="Language for translation (en/zh)")):
...
def run_server(host: str = "127.0.0.1", port: int = 8017, reload: bool = False):
